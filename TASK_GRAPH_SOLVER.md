...
- [`documentation/task-graph/real-guards/`](documentation/task-graph/real-guards/) — **design-stage, not yet implemented.** A new sibling environment, `real_task_graph_solver/`: the same node/DAG/executor machinery, but a node's Guard is a real, deterministic check (`mypy`, `ruff`, an architecture test, a package-build check) run against a small, purpose-built example codebase instead of a simulated `pass_probability` draw — grounded in `atomicguard`'s own `ContainerSubprocessGuard` and `autonomous-goal-net`'s guard-determinism hierarchy (`Bounded-Indeterminacy-Theory.md`). `RealCheckNode` is a strict subset of `atomicguard`'s real Action Pair notation (`docs/masters_report/chapters/appendix_b_notation.tex`) — the `a_guard_eff` slot alone, `a_gen`/`a_eff` both absent. Sensing only for this first phase — no repair yet, so `GuardFirstExecutor`'s free-check capability doesn't get a meaningful demonstration here, while `AOStarExecutor`'s cost composition and `PlanningExecutor`'s goal-directed short-circuit both get a real, wall-clock-backed one for the first time. Two phases follow, deliberately not combined into this one: a second, `atomicguard`-backed variant of the identical example (reusing its real `ActionPair`/`GuardInterface`/`EffectorInterface` rather than re-deriving retry/effector machinery it already has), and only then repair itself, using that variant's real Generator.
