...
- [`PATH_MAINTENANCE.md`](PATH_MAINTENANCE.md) / [`documentation/path-maintenance/`](documentation/path-maintenance/) — **implemented, three steps.** Orthogonal to D* Lite: a fixed path, walked as-is, with node-level repair rather than route repair. Step 1: a maze corridor (`maze_solver/agents/path_maintenance.py`, `CellState`/`get_cell_state()`/`inject_repairs()`/`repair_cell()`). Step 2: moved to an AND-only DAG (`path_maintenance/`, `PathGraphEnvironment`), reusing this module's `requires`-validation/`ready_nodes()` as a pattern, not its retry-economics-shaped `attempt()`/executors. Step 3: nodes gained a `PENDING`/`IN_PROGRESS`/`SUCCEEDED`/`FAILED` lifecycle sensed by a wait loop, driven by other agents rather than a scripted mutation. No D* Lite/LPA*, no `DualStateAgent`, no escalation yet — see [`documentation/task-graph/atomicguard-variant/`](documentation/task-graph/atomicguard-variant/) for where the two threads are expected to eventually converge.
