...
- [`documentation/task-graph/or-groups/`](documentation/task-graph/or-groups/) — **design-stage, not yet implemented.** Extends the environment with OR-groups (multiple nodes satisfying the same slot, only one needs to pass) and an explicit goal distinct from "every node satisfied" — grounded in `atomicguard`'s own real proposal for fixing its "single-exit corridor" RL bottleneck (`docs/archive/notes/2026-02-25T18-multi-path-rl-design.md`). This is what finally gives `AOStarExecutor` a real OR-choice to make and `DStarLiteExecutor` an actual alternate route to reroute through, neither of which the AND-only scenarios could demonstrate
