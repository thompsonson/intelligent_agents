...
## 4a. The atomicguard deep-research finding: the RV gap

A documented mapping from the atomicguard notes makes the Doyle link concrete and *prior* (not a retrospective stretch). The masters project plan names it explicitly: **the RV (Response Validation) gap** — *"classical agent theory has no category for validating responses. The guard fills it"* ([`docs/design/notes/masters_project_plan.md:45`](/home/mt/Projects/thompsonson/atomicguard/docs/design/notes/masters_project_plan.md)). The five-category Doyle taxonomy (IR/ER/RR/RV/CBF) derived from Doyle's 16 rational-psychology characteristics was built around this: the guard is the category rational psychology never needed, because it assumed deterministic action ([`rational_psychology_llm_agents.md`](/home/mt/Projects/thompsonson/atomicguard/docs/design/notes/rational_psychology_llm_agents.md)).

Three consequences for the belief-state thread:

- **INVALIDATE is the RV gap operationalized.** The guard decides which half of the management split fires: a failing post-guard revokes the negative fact (INVALIDATE — the Definition-47 cascade) *and* the merge updates the copy (RECONCILE), fused in one step. Doyle's deliberation reasons over reasons; the guard is where a justification's failure is detected against the world.
- **The freshness doctrine is reasoned deliberation over reasons, not chauvinistic utility.** A `:fresh-for N` bound is a *declared, revisable reason* for why a copy is trustworthy — the reasons layer — with the metadata (`sensed_at`, verdict) as the evidence the reasons point to. Scalar confidence is the "chauvinistic utility" temptation: the atomicguard history rejected it explicitly — *"explored logprobs as confidence signal — found confidence-accuracy mismatch: LLM can be confident and wrong. Internal signals formally insufficient → external validation necessary"* ([`masters_project_plan.md:59`](/home/mt/Projects/thompsonson/atomicguard/docs/design/notes/masters_project_plan.md)). Confidence is a decision signal, never a truth-maker.
- **The reserved stochastic-reasoning Kind has a documented reason.** D3 ([`dspddl_pddl_alignment.md`](/home/mt/Projects/thompsonson/atomicguard/docs/design/notes/dspddl_pddl_alignment.md)) classifies controllable/exogenous/static and *deliberately excludes* a stochastic/reasoning Kind — the generator channel stays out of the predicate-Kind system. That is exactly the reserved Kind in `stochastic-reasoning-kind.md`, with a justification: **Kind means who determines truth, and nothing determines the model's self-attested reasoning.** The DST/logprobs lineage ([`my_voice.md:251`](/home/mt/Projects/thompsonson/atomicguard/docs/design/notes/my_voice.md), [`masters_project_plan.md:59`](/home/mt/Projects/thompsonson/atomicguard/docs/design/notes/masters_project_plan.md)) held the generator channel is uncertainty-bearing — belief functions (Bel/Pl), not truth — which is why giving it a Kind would overclaim truth-determination.

Also worth noting for the archive's own belief-state story: `my_voice.md:213-217` records that the Doyle model was read, liked, then *discarded*, and "not knowingly connected to later work" — until the masters plan named the RV gap. A held belief re-connected when the world moved: the archive is a belief state that has itself been re-sensed.
...
- Atomicguard cross-references (verified): `docs/design/notes/masters_project_plan.md:45` (RV gap), `:59` (DST/logprobs, confidence-accuracy mismatch); `docs/design/notes/rational_psychology_llm_agents.md` (five-category Doyle taxonomy); `docs/design/notes/my_voice.md:213-217` (Doyle aside), `:251` (DST/logprobs); `docs/design/notes/voice/2025-05-14_debois_rational_agents_email.md` (practitioner rational-agents); `docs/design/notes/dspddl_pddl_alignment.md` (D3 — the Kind exclusion); `docs/masters_report.md:1935` (Doyle in the rational-agents bibliography).
