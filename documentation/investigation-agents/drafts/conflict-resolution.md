...
| `verified(claim)` | derived | reused from the parent draft — a guard checks the claim against ground truth |
...
**1. Verification** — reuses `VERIFY(finding)` from the parent draft exactly. A factual disagreement is a special case of the same neuro-symbolic seam: one side's claim is exogenous until a guard checks it. Nothing new here except recognizing the disagreement as this shape.
...
## A worked trace, continuing the parent draft.s example
...
- **Who classifies, and how, is still hand-waved.** "The shape of the claim determines it" is true but not yet a rule an orchestrator could execute — the same honesty the parent draft already gave `SELECT-NEXT` and `sufficient()`.
