# Draft: The Agent Design Process in practice — the minimal LLM loop

*Draft material for further work on modelling the agent loop. This walked the six-step process on the minimal LLM loop; it was removed from the [IA Series 13 post](../blog.md) when the worked example moved to the infra discovery agent, because the loop's world degenerates to exogenous and it has no belief state. The runnable live-server toy lives at `examples/minimal_llm_loop.hy`.*
