...
**Where this gets genuinely hard, not just a clean win:** `ReplicaSet` is not a registered `kind` in this environment's own `DSA-CATALOGUE[kubernetes]` (`schema.md` - Pods are grouped directly under `Deployment` via `DSA-K8S-PODSET`, matching the source catalogue's own choice not to give `ReplicaSet` a separate entry). So once `ReplicaSet` is discovered as `edge.from` here, `RELEVANT(BRIDGE-CATALOGUE[owns], edge.from, Ψ, belief_state)` has nowhere to go - `DSA-CATALOGUE[(kubernetes, ReplicaSet)]` doesn't exist. This is a sharper, concrete version of the finding raised on `atomicguard` PR #369: not just "the wrong catalogue might get selected for the new end," but "no catalogue may exist for the new end's kind at all," for a real, standard Kubernetes relationship, not a contrived one. Two ways this could go, illustrated here rather than resolved:
