...
| **Single/Multi-agent** | **Multi** | Corrected from an earlier draft of this table, which argued "Single-agent in the PEAS sense; not single-actor" from first principles before checking `atomicguard`'s own PEAS analysis directly. `platform_topology_peas_and_cli_actions.md`'s per-domain tables classify this **Multi** outright, for all three domains - GitHub's own scheduler and other automations, Kubernetes' controllers (ReplicaSet controller, kubelet, HPA, Argo Rollouts controller), GCP's control plane and every other identity with write access, are each named as independent agents acting on the same W the agent only observes. Deferring to that classification rather than defending a narrower reading invented without it - this table exists to be checked against real sources, not to relitigate one that already exists. Worth keeping visible, not dropped: this multi-agent-ness is what makes Dynamic (above) true in the first place, not a separate, unrelated fact - unlike `path_maintenance/job-lifecycle`'s multi-agent concurrency, which is about *other instances of this same kind of agent* cooperating on a shared plan, not independent external controllers this agent never coordinates with |
...
- **What concrete scenario this gets validated against.** `atomicguard-bridge/`'s `pipeline_fanout_lite` reuse doesn't fit here - fixed six-node topology, one implicit domain. A `scenario.md` needs its own decision. Partially narrowed by `atomicguard`'s own `topology_source_comparison_cartography_fix_bespoke.md`: bespoke DSAs against the catalogued domains (`github_actions`/`kubernetes`/`gcp`), not Cartography or Fix Inventory wholesale - both are stateless bulk/batch crawlers, directly conflicting with this environment's incremental, `Ψ`-scoped sensing, and Fix has no GitHub coverage at all. Cartography's own crawler code recommended as a reference for the K8s/GCP sensing calls, not a dependency. Still not written - what fixtures or real credentials a repo-local `scenario.md` would actually need is a separate, undecided question.
...
- `atomicguard`'s `docs/design/notes/topology_agent_function_requires_and_discovery_validation.md` - the `requires`/AND-join revision, and the `DSA-CATALOGUE`/`SELECT-NEXT` open questions `atomicguard-bridge/algorithm_fit.md` already cited. Both the `SWEEP-CLEARED` cycle-safety fix and the bidirectional-propagation fix this document's first draft prompted landed here (commits `07035745`, `c6a9f697`).
- `atomicguard`'s `docs/design/notes/platform_topology_peas_and_cli_actions.md` - Steps 1-2, the actual per-domain PEAS analysis this document's properties table is checked against (and corrected against - see the Single/Multi-agent row).
- `atomicguard`'s `docs/design/notes/topology_source_comparison_cartography_fix_bespoke.md` - Cartography/Fix Inventory/bespoke compared for sourcing real topology data; informs the "what concrete scenario" open item below.
