...
- **`result.path` records every move, including backtracks — full move log, not first-visit order.** `DiscoveryWalkResult.path` is now literally what the worked table's "To" column shows: 11 entries for `pipeline_fanout_lite` (10 moves plus the start), `merge-gate` and `commit` each appearing more than once. This is what let the visualization render frames 4–6 of `experiments/02_pipeline_fanout_backtracking.md`'s GIF, which have no equivalent in step 1's output at all.
- **Backtrack moves count against total cost.** `DiscoveryAgent.walk()` calls `get_move_cost()` on every move, forward or backtrack, and sums it into a new `DiscoveryWalkResult.total_cost` field — `10` on `pipeline_fanout_lite`, matching the move count exactly under the flat-1 cost model. This also resolves the step 1 review's "`get_move_cost()` is dead code" note: it's now the mechanism the agent itself uses to account for movement, not just a tested-in-isolation method.
- **Tie-break when backtracking: a LIFO stack, always to the immediate parent.** `DiscoveryAgent` pushes the current node before every forward move and pops on every backtrack — no richer tie-break was needed even to consider, since a stack has only one thing to pop. Still true that a scenario with a real "which grandparent" choice hasn't been built; the mechanism just turned out not to need one to exist first.
...
- [`../experiments/02_pipeline_fanout_backtracking.md`](../experiments/02_pipeline_fanout_backtracking.md) — this design run for real, frame-by-frame, with the GIF.
