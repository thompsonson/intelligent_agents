...
- **A real mistake, worth recording rather than quietly fixing.** The first version of this module ported `discovery/`'s own `build_networkx_graph(env)` almost verbatim - including its habit of peeking at every node in `env.nodes` to get a stable layout. Here, that peek is a real `sense_edges()` call: `animate_walk()` was making 90 real subprocess calls for this 15-frame/6-node walk (every frame re-sensing every node), instead of the 6 the walk itself needed - a real violation of this package's own "nothing gets sensed except through the walk" principle, not just wasted work, and a silent widening of a failing check's blast radius to nodes the walk never visited.

  **The same pattern in `discovery/`'s own version wasn't harmless either, and calling it that was wrong.** It costs nothing there (`.notifies` is a free field read, no subprocess involved), but "free" isn't "correct": `build_networkx_graph(env)` reads every node's true `notifies` directly, bypassing `sense_edges()` entirely - the exact god's-eye access `documentation/discovery/environment_design.md`'s own "Observable: Partially... no enumeration, no 'list all nodes'" says `DiscoveryAgent` itself is never given. Fixed in both places, the same way: `_walk_frames()`'s own already-sensed `known_edges` threaded through to `build_networkx_graph()`/`render()`, never the environment read directly. Confirmed directly: the real version now makes exactly 6 real senses, not 90 (`env._dag.get_all()` after `animate_walk()` has exactly 6 entries).

- **A second, smaller bug the fix itself surfaced.** `known_edges` only holds nodes the walk actually sensed - a node merely *named* in another node's `notifies` but never itself sensed (experiment 1's `unit-tests`/`integration-tests`, discovered down a branch the walk never took) isn't a key. The first fix crashed rendering experiment 1's GIF (`KeyError: 'is_goal'`) the moment it hit exactly that case. Fixed by adding such targets as placeholder nodes (`is_goal=False`, a value `_node_display_state()` never actually reads for an unvisited node) rather than assuming every edge target is already a sensed key.
