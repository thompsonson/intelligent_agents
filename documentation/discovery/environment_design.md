...
**Goal: a node with no `notifies`.** Rather than a named target known in advance, the goal is structurally discovered — a terminal the agent finds by walking into it. Caveat worth keeping visible: "no `notifies`" alone doesn't distinguish *the* goal from an incidental dead end if a scenario happens to have more than one such terminal. `scenario.md` needs exactly one reachable terminal for this step, to keep the goal condition unambiguous without needing extra machinery to pick among terminals. (Resolved differently, not just worked around, once backtracking exists: [`backtracking-exploration/algorithm_fit.md`](backtracking-exploration/algorithm_fit.md) reframes the objective from goal-seeking to full exploration, at which point multiple terminals stop being ambiguous at all — every one just gets visited.)
...
- **Traversal strategy** — resolved, twice over: `algorithm_fit.md` settled the original forward-committed policy, then `backtracking-exploration/algorithm_fit.md` superseded it with DFS-and-retrace, then `and-joins/algorithm_fit.md` added the readiness sweep on top. Nothing left open here.
- **Exact scenario topology** — resolved in `scenario.md`; reused unmodified by every later step rather than replaced.
- **`sense_edges` naming for what it returns** — still genuinely open. Confirmed as the method name at the time and never revisited since — three steps and 54 tests now exist against `DiscoveryNode`/`DiscoveryEnvironment`/`DiscoveryAgent`/`sense_edges`, which is its own argument for leaving it alone, but that's inertia, not a decision anyone actually made.
- **Growing edges after a node "finishes processing"** (e.g. a deploy spinning up more pods) — real, and explicitly out of scope for this whole `discovery/` arc as built so far, not just this step. Currently recorded only as a parenthetical in the properties table above ("Static/Dynamic"); flagged here so it has a visible home rather than staying buried in a table cell. Recombining this with `job-lifecycle`'s `PENDING`/`IN_PROGRESS`/`SUCCEEDED` machinery, as originally sketched when this idea first came up, is still nothing more than a sketch.
