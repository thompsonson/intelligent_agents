...
**Resolved:** `ensure()` becomes a new, separately-named executor, `PlanningExecutor` — `AOStarExecutor` keeps its existing forward-frontier behavior and existing tests unchanged. `PlanningExecutor` is not "AO*, corrected" and isn't presented as one; it's its own thing, reusing AO*'s AND-composition and OR-group-pruning ideas but arrived at top-down rather than forward-frontier. Worth a one-line cross-reference on `AOStarExecutor` itself pointing here, so a reader who notices the orphan-visiting behavior isn't left thinking it's an oversight.
