...
- **Which failure mode's repair gets built first.** Recommended `lint` (deterministic, `ruff --fix`, no LLM) above, not committed.
- **Minimal `Context`/`AmbientEnvironment` wiring.** `AmbientEnvironment.repository` is a required, non-optional `ArtifactDAGInterface` - `InMemoryArtifactDAG` (`infrastructure/persistence/memory.py`) is the obvious candidate for a workflow that doesn't need real persistence, but the exact minimal construction isn't worked out here.
...
- **Whether an LLM-based repair path needs a real LLM endpoint configured for this repo's own tests**, or should be exercised with `infrastructure/llm/mock.py`'s mock model the way this project's own `llm_agents/` work already does for its own LLM-backed tests. Leaning toward the mock for automated tests, a real endpoint only for the manually-run demonstration/animation - not decided here.
- **Naming.** Working names only (`atomicguard_task_graph_solver` or similar), per this project's own history of renaming once code exists and a clearer name suggests itself.
