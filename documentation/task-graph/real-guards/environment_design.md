...
Three phases follow now, not two — the design doc for the second phase (`documentation/task-graph/atomicguard-variant/environment_design.md`) found something worth correcting here rather than leaving stale: that phase is **still sensing-only**, not where repair gets built. `atomicguard`'s `CommandTemplateGenerator` (the generator its own real sysadmin checks use, e.g. `disk_check.dspddl`) is deterministic - string templating, no LLM - so wiring real `ActionPair`/`WorkflowOrchestrator` machinery in place of this document's bespoke `subprocess.run` call doesn't, by itself, introduce anything nondeterministic.

1. **A second, independent example variant that actually depends on `atomicguard`** — same fixture package, same DAG, same manufactured states, driven by `atomicguard`'s real `ActionPair`/`WorkflowOrchestrator` machinery (which turns out to already have native `requires`/`requires_any`/`group`/`goal`/`required` support - almost exactly `or-groups/` and `goal-directed-planning/`'s primitives, arrived at independently) instead of this document's own minimal classes. `atomicguard` becomes an actual dependency for that variant, not just a repo read for reference — new for this project, detailed in that variant's own design doc.
2. **Repair itself** — a genuinely stochastic Generator, replacing `CommandTemplateGenerator` for the four failure-mode Action Pairs specifically, using `atomicguard`'s existing `ActionPair.execute()` four-phase transaction and `WorkflowOrchestrator`'s real retry/escalation machinery rather than anything bespoke. This is where `rmax`/`r_patience` (a repair attempt genuinely can differ from the last one), `retry_flavor="repair"` (so `LRTAStarLearner` has something real to learn a cost for), and `GuardFirstExecutor`'s free-check capability (repair now has a real cost worth skipping) all become meaningful again — none of that is designed here, deliberately; this document is just the sensing half, built to be solid on its own before anything nondeterministic is added on top of it.
