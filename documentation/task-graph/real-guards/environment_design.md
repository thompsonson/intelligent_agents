...
| `build-check` | `python -m build --sdist --wheel` (or `twine check`) | `publish_broken` |
| `release-ready` (**goal**) | `["true"]` - a deliberate no-op | Never independently fails; satisfied once all five above are |

`release-ready` having no real check of its own, beyond its `requires`, is a direct model of a real, common thing: GitHub branch protection's "require status checks to pass" is exactly this — an aggregation gate with no independent check beyond "are these other checks green." Not invented for this document; named here because it's the honest justification for a synthetic AND-join node that itself does nothing.
...
Explicitly not started here, named so this document doesn't read as a dead end: once this environment exists, is documented, and has scripts to manage it (build the fixtures, reset state, run a check), the next phase is giving the DS-PDDL Action Pair's Generator something real to do — an LLM (or scripted patch, decided later) that, on a real `FATAL`, attempts to actually fix `typing_broken`/`lint_broken`/`architecture_broken`/`publish_broken` for real, with this environment's checks as its Guards. That reintroduces everything this document deliberately left out: `rmax`/`r_patience` (a repair attempt genuinely can differ from the last one), `retry_flavor="repair"` (so `LRTAStarLearner` has something real to learn a cost for), and `GuardFirstExecutor`'s free-check capability (repair now has a cost worth skipping). None of that is designed here — this document is deliberately just the sensing half, built to be solid before anything nondeterministic is added on top of it.
