...
One phase follows, not two or three - the design doc for it (`documentation/task-graph/atomicguard-variant/environment_design.md`) went through two corrections worth being honest about rather than smoothing over: first this section claimed that phase was where repair gets built; then, on closer reading of `atomicguard`'s deterministic `CommandTemplateGenerator`, it was corrected to "still sensing-only, repair deferred further." Both were superseded once the user directing this work clarified the actual intent: keep this repo's own executors doing the searching (not `atomicguard`'s `WorkflowOrchestrator`, which turns out to already have native `requires`/`requires_any`/`group`/`goal`/`required` support - almost exactly `or-groups/` and `goal-directed-planning/`'s primitives, arrived at independently - but would replace the thing this project teaches, not complement it), and give each node a real, individually-callable Action Pair whose Guard can be checked for free and, if unsatisfied, repaired for real. That's one phase, not a further split:

**A second, independent example variant that actually depends on `atomicguard`** — same fixture package, same DAG, same manufactured states, but each node now wraps a real `ActionPair` (Guard always; Generator+Effector where repair is being demonstrated) instead of this document's own minimal `RealCheckNode`/`subprocess.run`. `atomicguard`'s own `infrastructure/gym/precommit_generators.py` (`LLMContainerFixGenerator`) and its `PreCommitGym`'s deterministic `ruff --fix` auto-repair are the concrete, already-built repair generators this variant draws on, rather than inventing new ones. `atomicguard` becomes an actual dependency for that variant, not just a repo read for reference — new for this project, detailed in that variant's own design doc. This is where `rmax`/`r_patience`, `retry_flavor="repair"` (so `LRTAStarLearner` has something real to learn a cost for), and `GuardFirstExecutor`'s free-check-before-real-repair capability all become meaningful for the first time — none of that is designed here, deliberately; this document is just the sensing half.
...
- **The `atomicguard`-backed second variant now has its own design doc:** [`documentation/task-graph/atomicguard-variant/environment_design.md`](../atomicguard-variant/environment_design.md).
