...
[`environment_specification.md`](environment_specification.md) (Step 1):
checked directly, no file in the original `documentation/infra-discovery/`
owns this content. `algorithm_fit.md` (Step 4) reasons extensively *about*
...
documents (`environment_analysis.md`, `algorithm_fit.md`) but never
translated into this track's own words the way `environment_analysis.md`
translated the Node/Edge ontology from the same source document.

## What translating this file for real would need to cover

Mirroring what `environment_analysis.md`/`schema.md` already did for the
...
- **Percepts** - what a DSA invocation's `Artifact` looks like as a percept
  here, parallel to `schema.md`'s `Facet` shape.
- **Actions** - the DSA vocabulary (`INVOKE(dsa, subject)`), and how
  `pending`/`SELECT-NEXT` structure the action space - `algorithm_fit.md`
...
- **The pseudocode itself** - `AGENT-FUNCTION`'s current, revised form
  ...
  `algorithm_fit.md`'s Mermaid sequence diagrams did for
...
- **Cost features** - named in the source document, never checked against
  ...
  `schema.md`'s `DSA-CATALOGUE` tables checked the ontology against them.

This is, alongside `environment_specification.md`, the retrofit's other
...
- [`algorithm_fit.md`](algorithm_fit.md) - Step 4; reasons about this step's output without this file existing to reason from.
