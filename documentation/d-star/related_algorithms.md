...
- ~~Whether a toy example is needed for repairing a *node's* state as distinct from repairing the *plan*~~ — resolved: [`documentation/path-maintenance/`](../path-maintenance/) now exists, holding [`environment_design.md`](../path-maintenance/environment_design.md). It deliberately doesn't use D* Lite, LPA*, or anything else in this table — the maze's route is fixed, only a cell's content needs fixing, which is MAPE-K/self-stabilization territory, not incremental-replanning territory.
