...
**Design, scenario, and algorithm fit**: [`environment_design.md`](documentation/discovery/atomicguard-bridge/environment_design.md) — environment properties (deltas from `discovery/`'s own table called out explicitly, not silently inherited) and the node-ownership resolution (local guard-checked state vs. cross-node `requires`/cleared bookkeeping, matching `atomicguard`'s own `WorkflowStep`/`WorkflowState` split); [`scenario.md`](documentation/discovery/atomicguard-bridge/scenario.md) — the fixture-backed topology and why `cat`-over-JSON, not a faked `Generator`; [`algorithm_fit.md`](documentation/discovery/atomicguard-bridge/algorithm_fit.md) — why `DiscoveryAgent`'s algorithm still fits argued properly (not just asserted by a passing test), the real over-sensing bug this exposed, and DSA selection (a node with more than one candidate check) kept explicitly open rather than foreclosed by this step's single-`check_action_pair` node shape.
...
- [`documentation/discovery/atomicguard-bridge/environment_design.md`](documentation/discovery/atomicguard-bridge/environment_design.md) — `StatefulDiscoveryNode`/`StatefulDiscoveryEnvironment`'s shape, its environment-properties table, and the node-ownership resolution.
- [`documentation/discovery/atomicguard-bridge/scenario.md`](documentation/discovery/atomicguard-bridge/scenario.md) / [`algorithm_fit.md`](documentation/discovery/atomicguard-bridge/algorithm_fit.md) — the fixture-backed topology, the real, unmodified-`DiscoveryAgent` fit argument against `real_discovery/atomicguard_backed/`, and DSA selection (a node with more than one candidate check) named as the most significant open item.
