...
`LRTAStarLearner` learns `h_table` — a per-node cost estimate — for `retry_flavor="repair"` nodes only, over repeated trials via an `env_factory(trial_index)` callable. Each trial starts fresh; what persists across trials is the learned heuristic, not the environment state. The update rule is `h(s) ← max(h(s), retries_spent(s))`: the estimate can only grow, never shrink, and stops moving once the true worst case has actually been observed.
