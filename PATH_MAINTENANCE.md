...
- **Step 3 (job lifecycle, multi-agent sensing): design in progress, deliberately not started.** Builds on step 2, now that it exists. Environment properties and terminology settled (dynamic, multi-agent, deterministic, sequential, known); the exact lifecycle-resolution mechanism and whether `FAILED` is in scope yet are still open — see `job-lifecycle/environment_design.md`'s "Not decided" section.
